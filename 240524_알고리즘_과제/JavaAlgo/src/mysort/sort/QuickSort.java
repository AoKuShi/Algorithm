...
public class QuickSort extends Sort { // Sort 추상클래스를 상속받아 QuickSort 클래스를 생성
	...
	int partition(int[] dataList, int start, int end) { // 배열을 분할하고 피벗의 최종 위치를 반환
		int pivot = dataList[end]; // 마지막 요소를 피벗으로 설정
		int leftEnd = start - 1; // 왼쪽 부분의 끝 인덱스를 초기화
		for (int i = start; i <= end - 1; i++) { // 시작부터 끝 - 1까지 반복
			if (dataList[i] < pivot) { // 현재 요소가 피벗보다 작으면
